...
def test_non_mapped_as_dataclass_model_error():
    """Test that creating a DTO from a non-MappedAsDataclass model raises TypeError."""
    class NonMappedAsDataclassModel(Base):
        __tablename__ = "non_mapped_as_dataclass_test"
        id: Mapped[int] = mapped_column(Integer, primary_key=True)
        name: Mapped[str] = mapped_column(String)

    with pytest.raises(TypeError, match="Cannot create DTO from abstract or unmapped SQLAlchemy model"):
        class InvalidDTO(DTO[NonMappedAsDataclassModel]):
            pass


def test_no_tablename_model_error():
    """Test that creating a DTO from a model without __tablename__ raises TypeError."""
    class NoTableNameModel(Base):
        # Intentionally lacks __tablename__
        id: Mapped[int] = mapped_column(Integer, primary_key=True)
        name: Mapped[str] = mapped_column(String)

    with pytest.raises(TypeError, match="Cannot create DTO from abstract or unmapped SQLAlchemy model"):
        class InvalidDTO(DTO[NoTableNameModel]):
            pass
...
def test_mapping_to_incompatible_type():
    ...
    class IncompatibleMappedDTO(DTO[User]):
        config = DTOConfig(
            mapped={User.age: str} # Mapping int to str
        )
